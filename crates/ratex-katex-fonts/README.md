...
Embeds the KaTeX `.ttf` files used by RaTeX for bundled-font builds (`ratex-svg`, optional `ratex-render`, `ratex-pdf`). Font files ship inside this crate so crates.io releases compile without a monorepo `fonts/` directory.
